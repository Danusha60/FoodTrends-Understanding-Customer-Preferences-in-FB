Nutrition, Lifestyle & Food Analytics Dashboard
📌 Project Overview
This project presents an interactive data visualization dashboard built using Power BI to analyze food preferences, nutrition patterns, lifestyle habits, and obesity-related indicators.

The dashboard provides insights into:
Dietary preferences and restaurant trends,Nutrition and physical activity patterns,Breakfast recommendations and nutrient comparision,Lifestyle impact on health metrics like BMI, cholesterol and disease risk

Objectives:
Analyze food consumption patterns and dietary choices
Understand the relationship between lifestyle and health indicators
Identify trends in restaurant preferences and ordering behavior
Provide meaningful insights for better health and nutrition decisions
<img width="1317" height="733" alt="image" src="https://github.com/user-attachments/assets/ea7c5eef-83a2-46ab-9f3f-9d945903f857" />
1️⃣ Dietary Preferences & Averages
This section focuses on restaurant data and user food choices.
Key Insights:
⭐ Average rating: 3.51 / 5
💰 Average cost for two: ₹540
📊 Total number of ratings: 1M
🍽️ Most popular restaurant type: Quick Bites
📍 Area-wise distribution of restaurants and online ordering trends
📉 Relationship between cost and ratings
<img width="1323" height="736" alt="image" src="https://github.com/user-attachments/assets/a29d701c-c512-408b-88e7-465e6700b60a" />

2️⃣ Nutrition, Physical Activity & Obesity Analysis
Key Insights:
📊 Average health score: 31.03
👥 Age groups show relatively stable health values
🥗 Class-based comparison (OWS, PA, FV)
📈 Trends of health metrics over time
🔍 Comparison across age groups and categories
<img width="1320" height="735" alt="image" src="https://github.com/user-attachments/assets/2e69b1dd-140c-463b-a606-f33d75681b00" />

3️⃣ Breakfast Suggestions Analysis
Key Insights:
🥑 High-calorie foods: Avocado Toast, Breakfast Burrito
🥣 High-protein options: Oatmeal, Protein Pancakes
💸 Price vs Nutrition comparison
🍬 Sugar vs Price relationship
👥 Distribution across dietary preferences (Omnivore, Vegetarian, Vegan, Pescatarian)
<img width="1313" height="736" alt="image" src="https://github.com/user-attachments/assets/c6aca889-b49d-49fb-9ea0-e69997b9607c" />

4️⃣ Lifestyle & Nutrition Impact
Key Insights:
⚖️ Average BMI: 29.02
🧬 Average cholesterol: 224.30
🔥 Average calories consumed: 2.60K
🚶 Average daily steps: 10.48K
🚬 Smoking distribution analysis
📈 Disease risk trends across age
👨‍⚕️ Gender-based comparison of health metrics

🛠️ Tools & Technologies
Power BI – Data visualization and dashboard creation
Excel / CSV – Data preprocessing
DAX – Calculations and measures

📈 Use Cases
Health and nutrition analysis
Food industry insights
Lifestyle behavior studies
Data visualization projects for students

🚀 Conclusion
This dashboard helps in understanding how diet, lifestyle, and physical activity impact overall health. It also provides valuable insights into food trends and consumer behavior, making it useful for both health analysis and business decision-making.
